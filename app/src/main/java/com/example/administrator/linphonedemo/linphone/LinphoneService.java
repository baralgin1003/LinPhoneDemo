...
import com.example.administrator.linphonedemo.linphone.compatibility.Compatibility;
import com.example.administrator.linphonedemo.linphone.ui.LinphoneOverlay;
	...
	*/
	class ActivityMonitor implements Application.ActivityLifecycleCallbacks {
		...
		public synchronized void onActivityCreated(Activity activity, Bundle savedInstanceState) {
			KLog.i("Activity created:" + activity);
		...
		public void onActivityStarted(Activity activity) {
			KLog.i("Activity started:" + activity);
		}

		@Override
		public synchronized void onActivityResumed(Activity activity) {
			KLog.i("Activity resumed:" + activity);
			if (activities.contains(activity)) {
				mRunningActivities++;
				KLog.i("runningActivities=" + mRunningActivities);
				checkActivity();
			}
		...
		public synchronized void onActivityPaused(Activity activity) {
			KLog.i("Activity paused:" + activity);
			if (activities.contains(activity)) {
				mRunningActivities--;
				KLog.i("runningActivities=" + mRunningActivities);
				checkActivity();
		...
		public void onActivityStopped(Activity activity) {
			KLog.i(TAG, "ActivityMonitor.onActivityStopped  " + "activity = [" + activity + "]");
		}
		...
		public synchronized void onActivityDestroyed(Activity activity) {
			KLog.i(TAG, "ActivityMonitor.onActivityDestroyed  " + "activity = [" + activity + "]");
			if (activities.contains(activity)) {
	...
	protected void onBackgroundMode(){
		KLog.i("App has entered background mode");
	...
	protected void onForegroundMode() {
		KLog.i("App has left background mode");
	}
	...
	public void onCreate() {
		KLog.i(TAG, "========== LinphoneService.onCreate");
		super.onCreate();
		...
		KLog.i(TAG, "========== LinphoneManager初始实例化 = " + LinphoneManager.getInstance());
		...
		LinphoneManager.getLc().addListener(mListener = new LinphoneCoreListenerBase() {
			...
			public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
				KLog.i(TAG, "========== LinphoneService.callState  " + "lc = [" + lc + "], call = [" + call + "], state = [" + state + "], message = [" + message + "]");
                KLog.i(TAG, "========== call.getRemoteUserAgent() = " + call.getRemoteUserAgent());
				if (instance == null) {
					KLog.i("Service not ready, discarding call state change to ",state.toString());
					return;
				}
			...
			public void globalState(LinphoneCore lc, LinphoneCore.GlobalState state, String message) {
				KLog.i(TAG, "========== LinphoneService.globalState  " + "lc = [" + lc + "], state = [" + state + "], message = [" + message + "]");
			...
			public void registrationState(LinphoneCore lc, LinphoneProxyConfig cfg, LinphoneCore.RegistrationState state, String smessage) {
				KLog.i(TAG, "========== LinphoneService.registrationState  " + "lc = [" + lc + "], cfg = [" + cfg + "], state = [" + state + "], smessage = [" + smessage + "]");
				KLog.i(TAG, "========== cfg = " + cfg.getDomain());
//				if (instance == null) {
//					KLog.i("Service not ready, discarding registration state change to ",state.toString());
//					return;
//				}
	...
	public void createOverlay() {
		KLog.i(TAG, "========== LinphoneService.createOverlay  " + "");
	...
	public void destroyOverlay() {
		KLog.i(TAG, "========== LinphoneService.destroyOverlay  " + "");
		if (mOverlay != null) {
	...
	private synchronized void setIncallIcon(IncallIconState state) {
		KLog.i(TAG, "========== LinphoneService.setIncallIcon  " + "state = [" + state + "]");
        KLog.i(TAG, "========== mCurrentIncallIconState = " + mCurrentIncallIconState);
        if (state == mCurrentIncallIconState) return;
        KLog.i(TAG,"==========  state != mCurrentIncallIconState");
        ...
        KLog.i(TAG, "========== mIncallNotif = " + mIncallNotif);
		notifyWrapper(INCALL_NOTIF_ID, mIncallNotif);
	}

	public void refreshIncallIcon(LinphoneCall currentCall) {
		KLog.i(TAG, "========== LinphoneService.refreshIncallIcon  " + "currentCall = [" + currentCall + "]");
	...
	public void addNotification(Intent onClickIntent, int iconResourceID, String title, String message) {
		KLog.i(TAG, "========== LinphoneService.addNotification  " + "onClickIntent = [" + onClickIntent + "], iconResourceID = [" + iconResourceID + "], title = [" + title + "], message = [" + message + "]");
	...
	public void removeCustomNotification() {
		KLog.i(TAG, "========== LinphoneService.removeCustomNotification  " + "");
		mNM.cancel(CUSTOM_NOTIF_ID);
		resetIntentLaunchedOnNotificationClick();
	}

	public void displayMessageNotification(String fromSipUri, String fromName, String message) {
		KLog.i(TAG, "========== LinphoneService.displayMessageNotification  " + "fromSipUri = [" + fromSipUri + "], fromName = [" + fromName + "], message = [" + message + "]");
	...
	public void displayInappNotification(String message) {
		KLog.i(TAG, "========== LinphoneService.displayInappNotification  " + "message = [" + message + "]");
		Intent notifIntent = new Intent(this, LinphoneActivity.class);
	...
	public void removeMessageNotification() {
		KLog.i(TAG, "========== LinphoneService.removeMessageNotification  " + "");
	...
	void invokeMethod(Method method, Object[] args) {
		KLog.i(TAG, "========== LinphoneService.invokeMethod  " + "method = [" + method + "], args = [" + args + "]");
		try {
	...
	void startForegroundCompat(int id, Notification notification) {
		KLog.i(TAG, "========== LinphoneService.startForegroundCompat  " + "id = [" + id + "], notification = [" + notification + "]");
	...
	void stopForegroundCompat(int id) {
		KLog.i(TAG, "========== LinphoneService.stopForegroundCompat  " + "id = [" + id + "]");
	...
	private void dumpDeviceInformation() {
 		KLog.i(TAG, "========== LinphoneService.dumpDeviceInformation  " + "");
	...
	private void dumpInstalledLinphoneInformation() {
		KLog.i(TAG, "========== LinphoneService.dumpInstalledLinphoneInformation  " + "");
		...
		if (info != null) {
			KLog.i("Linphone version is ", info.versionName + " (" + info.versionCode + ")");
		} else {
			KLog.i("Linphone version is unknown");
		}
	}
	...
	private synchronized void sendNotification(int level, int textId) {
		KLog.i(TAG, "========== LinphoneService.sendNotification  " + "level = [" + level + "], textId = [" + textId + "]");
	...
	private synchronized void notifyWrapper(int id, Notification notification) {
        KLog.i(TAG, "========== LinphoneService.notifyWrapper  " + "id = [" + id + "], notification = [" + notification + "]");
		if (instance != null && notification != null) {
			mNM.notify(id, notification);
		} else {
			KLog.i("Service not ready, discarding notification");
		}
	}
	...
	public void setActivityToLaunchOnIncomingReceived(String activityName) {
		KLog.i(TAG, "========== LinphoneService.setActivityToLaunchOnIncomingReceived  " + "activityName = [" + activityName + "]");
		try {
	...
	private void resetIntentLaunchedOnNotificationClick() {
		KLog.i(TAG, "========== LinphoneService.resetIntentLaunchedOnNotificationClick  " + "");
		Intent notifIntent = new Intent(this, incomingReceivedActivity);
		mNotifContentIntent = PendingIntent.getActivity(this, 0, notifIntent, PendingIntent.FLAG_UPDATE_CURRENT);
	...
	protected void onIncomingReceived() {
		KLog.i(TAG, "========== LinphoneService.onIncomingReceived  " + "");
		//wakeup linphone
